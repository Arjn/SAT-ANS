...
class UKF(object):
    ...
    def __init__(self, dim_x, dim_z, dt, hx, fx, points,
        ...
        #THE OBSERVATION SIGMAS ARE DEFINED DYNAMICALLY DURING RUN-TIME TO ACCOUNT FOR CHANGINING OBSERVATION CONDITIONS
    ...
    def predict(self, dt=None,  UT=None, fx_args=()):
        ...
        self.x, self.P = UT(len(self.x),self.sigmas_f, self.Wm, self.Wc, self.Q,
    ...
    def update(self, z, num_measurements, R=None, UT=None, hx_args=()):
        ...
        self._dim_z = self.dim_z
        if len(num_measurements) > 0:
            #can't update KF without any measurements
            # print('FILTER UPDATE')
            if z is None:
                return
            ...
            for lib in z:
                #remove the library items= names
                if type(list(lib.values())[0]) is np.ndarray:
                    z_temp.extend(flatten(list(lib.values())))
                else:
                    z_temp.extend(list(lib.values()))
            ...
            if self.dim_z != len(diag_R):
                ...
                temp = np.zeros([self.dim_z, self.dim_z])
                _len = len(np.diagonal(R))
                for i in range(0, self.dim_z, _len):
            ...
            for i in range(self._num_sigmas): # runs through the sigmas
                ...
                for func in self.hx: # runs through the functions belonging to the different sensors
                    ...
                    if type(temp[0]) is np.ndarray:
                        ...
                        func_output_temp = temp
                    sig_h_temp.extend(func_output_temp)
                self.sigmas_h[i] = np.array(sig_h_temp)

            # mean and covariance of prediction passed through unscented transform
            zp, Pz = UT(self.num_meas, self.sigmas_h, self.Wm, self.Wc, R, self.z_mean, self.residual_z)
            ...
            for i in range(self._num_sigmas):
                ...
                j = 0
                q = 0
                for func in self.residual_z:
                    dz_temp.extend(func(self.sigmas_h[i, j:j+num_measurements[q]], zp[j:j+num_measurements[q]]))
                    j += num_measurements[q]
                    q += 1
                # for func in self.residual_z: # runs through the funtions for the residuals of the different sensors
                #     dz_temp.append(func(self.sigmas_h[i], zp))
                dz = np.array(dz_temp)
                Pxz += self.Wc[i] * outer(dx, dz)


            self.K = dot(Pxz, inv(Pz)) #OLD - using pseudo inverse as normal inverse leads to errors
            # s = cholesky(Pz)
            # self.K = dot(Pxz, s*s.T)  # Kalman gain
            j = 0; q = 0
            self.y = []
            for func in self.residual_z:
                self.y.extend(func(z[j:j + num_measurements[q]], zp[j:j + num_measurements[q]]))
                j += num_measurements[q]
                q +=1
...
def unscented_transform(num_measurements, sigmas, Wm, Wc, noise_cov=None,
    ...
    if mean_fn is None:
        ...
        # for func in mean_fn:
        #     x.append(func(sigmas, Wm))
        i = 0
        j = 0
        for func in mean_fn:
            x.extend(func(sigmas[:,i:i+num_measurements[j]], Wm))
            i += num_measurements[j]
            j += 1
    x = np.array(x)
    ...
    if residual_fn is None:
        ...
        for k in range(kmax):
            ...
            i = 0
            j = 0
            if type(residual_fn) != np.ufunc:
                for func in residual_fn:
                    y.extend(func(sigmas[k][i:i+num_measurements[j]], x[i:i+num_measurements[j]]))
                    i += num_measurements[j]
                    j += 1
            else:
                y.extend(residual_fn(sigmas[k][i:i + num_measurements], x[i:i + num_measurements]))
                i += num_measurements
            y = np.array(y)
            P += Wc[k] * np.outer(y, y)
