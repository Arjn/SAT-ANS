...
class Main(object):
    def __init__(self, TIMING, ORBITAL, SENSORS, NAVIGATION, ONBOARD_CLOCK, updates=None):
        ...
        self.updates = updates if updates is not None else None
    ...
    def run_simulation(self):
        ...
        update = self.updates if self.updates is not None else True
        ...
        for iter in tqdm(range(int(self.sim_length.value - self.global_timer.value)//int(self.global_dt.value))):
            ...
            if self.nav_timer >= self.nav_dt:
                ...
                self.MSE_P[nav_iter] = (temp)
                temp = (np.sqrt((self.Orbit.state[3:] - self.nav_module.filter_state[3:])**2))
                self.MSE_V[nav_iter] = temp
                self.filter_covar[nav_iter] = (np.diag(self.nav_module.ukf.P))
                test_storage[nav_iter] = (temp.tolist())
                timer_storage[nav_iter] = (self.global_timer.value)
                storage_true[nav_iter] = (self.Orbit.state)
                storage_filter[nav_iter] = (self.nav_module.filter_state)
